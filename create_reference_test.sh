...
mkdir -p "$TEST_DIR"
cat > "$TEST_DIR/DspReferenceAccuracyTest.kt" <<'KOTLIN'
...
class DspReferenceAccuracyTest {
    ...
    fun compressorReferenceTest_GainReduction() {
        ...
        processor.attackMs = 1f // Fast attack to settle quicker
        processor.releaseMs = 10f
    ...
    fun limiterReferenceTest_Transient() {
        ...
        processor.flush()
        val aboveInput = createTransientBuffer(0.5, 1.5) // Exceeds 1.0, but ByteBuffer creation coerces or we just test the peak
        // Wait, if it exceeds 1.0, short cast wraps? My createTransientBuffer uses 1.5, which will overflow short?
        // Let's use 1.0 peak (0 dBFS) to test above ceiling (-1.0 dBFS).
    ...
    fun expanderReferenceTest() {
        ...
        for (levelDb in levels) {
            ...
            if (inRms < -40.0) {
                val expectedGainChange = (inRms - -40.0) * (2f - 1f)
